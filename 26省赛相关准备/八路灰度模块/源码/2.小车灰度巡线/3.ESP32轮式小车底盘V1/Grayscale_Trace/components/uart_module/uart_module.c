...
void uart0_init() {
    ...
    uart_param_config(UART_NUM_0, &uart_config);// 应用配置参数 | Apply configuration
    uart_set_pin(UART_NUM_0, UART0_TX_PIN, UART0_RX_PIN, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE);// 设置GPIO引脚 | Set GPIO pins
    uart_driver_install(UART_NUM_0, 2048, 2048, 50, &uart_queue, 0);// 安装UART驱动 | Install UART driver
}
        ...
        if (xQueueReceive(uart_queue, (void *)&event, (TickType_t)portMAX_DELAY)) {
            switch (event.type) 
            {
                case UART_DATA:// 数据到达事件 | Data received event
                    int len = uart_read_bytes(UART_NUM_0, dtmp, event.size, portMAX_DELAY);// 读取UART数据 | Read UART data
                ...
                case UART_BUFFER_FULL:// 缓冲区满处理 | Buffer full handling
                    uart_flush_input(UART_NUM_0);// 清空输入缓冲区 | Clear input buffer
                    xQueueReset(uart_queue);    // 重置事件队列 | Reset event queue
                    break;
                case UART_FIFO_OVF:// FIFO溢出处理 | FIFO overflow handling
                    uart_flush_input(UART_NUM_0);// 清空硬件FIFO | Clear hardware FIFO
                    xQueueReset(uart_queue);    // 重置事件队列 | Reset event queue
                    break;
                default:
                    break;

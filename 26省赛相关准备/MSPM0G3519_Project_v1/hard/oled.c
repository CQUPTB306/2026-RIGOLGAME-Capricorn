...
/*==================== I2C 地址配置 ====================*/
/** @brief OLED I2C 从机写地址
 *  @note  常见地址: 0x78 (SA0=GND, 推荐) 或 0x7A (SA0=VCC)
 *          如 OLED 不响应, 请检查模块背面 SA0 电阻, 尝试改为 0x7A */
 ...
 */
static uint8_t OLED_WriteCommand(SoftI2C_Obj *obj, uint8_t Command)
{
    uint8_t err = 0;
    SoftI2C_Start(obj);
    SoftI2C_SendByte(obj, OLED_I2C_ADDR);
    err |= SoftI2C_WaitAck(obj);
...
/**
 * @brief  OLED 批量写数据 (单次 I2C 事务发送多字节, 利用 SSD1306 自动列递增)
 * @param  obj   软件 I2C 总线对象指针
 * @param  Data  数据缓冲区指针
 * @param  Len   数据长度 (字节)
 */
static uint8_t OLED_WriteBulkData(SoftI2C_Obj *obj, const uint8_t *Data, uint8_t Len)
{
    uint8_t err = 0;
    SoftI2C_Start(obj);
    SoftI2C_SendByte(obj, OLED_I2C_ADDR);
...
void OLED_Init(SoftI2C_Obj *obj)
{
    ...
    /* SSD1306 初始化序列 (带重试 + ACK 校验) */
    while (retry--)
    {
        err = 0;

        err |= OLED_WriteCommand(obj, 0xAE);  /* 关闭显示 (Display OFF) */

        err |= OLED_WriteCommand(obj, 0xD5);  /* 设置时钟分频比/振荡器频率 */
        err |= OLED_WriteCommand(obj, 0x80);  /* 默认值: 分频比=1, 频率≈370kHz */

        err |= OLED_WriteCommand(obj, 0xA8);  /* 设置多路复用比 */
        err |= OLED_WriteCommand(obj, 0x3F);  /* 64 COM (128×64) */

        err |= OLED_WriteCommand(obj, 0xD3);  /* 设置显示偏移 */
        err |= OLED_WriteCommand(obj, 0x00);  /* 偏移 = 0 */

        err |= OLED_WriteCommand(obj, 0x40);  /* 设置显示起始行 = 0 */

        err |= OLED_WriteCommand(obj, 0xA1);  /* 段重映射: 列 127 → SEG0 (左右正常) */
        err |= OLED_WriteCommand(obj, 0xC8);  /* COM 扫描方向: COM[N-1] → COM0 (上下正常) */

        err |= OLED_WriteCommand(obj, 0xDA);  /* COM 引脚硬件配置 */
        err |= OLED_WriteCommand(obj, 0x12);  /* 备选 COM 引脚配置 (128×64) */

        err |= OLED_WriteCommand(obj, 0x81);  /* 设置对比度 */
        err |= OLED_WriteCommand(obj, 0xCF);  /* 对比度值 (0~255, 默认 0x7F) */

        err |= OLED_WriteCommand(obj, 0xD9);  /* 设置预充电周期 */
        err |= OLED_WriteCommand(obj, 0xF1);  /* Phase1=1, Phase2=15 */

        err |= OLED_WriteCommand(obj, 0xDB);  /* 设置 VCOMH 电压 */
        err |= OLED_WriteCommand(obj, 0x30);  /* ~0.77×VCC */

        err |= OLED_WriteCommand(obj, 0xA4);  /* 全屏点亮: 跟随 GDDRAM 内容 */

        err |= OLED_WriteCommand(obj, 0xA6);  /* 正常显示 (非反色) */

        err |= OLED_WriteCommand(obj, 0x8D);  /* 启用电荷泵 */
        err |= OLED_WriteCommand(obj, 0x14);  /* 电荷泵开启 (3.3V 供电必需) */

        err |= OLED_WriteCommand(obj, 0xAF);  /* 开启显示 (Display ON) */

        if (err == 0) break;  /* 全部 ACK 成功 → 退出重试 */
        delay_ms(10);         /* 失败 → 等待后重试 */
    }
...
{
    /* 上半部分 (8 像素) — 批量写入, 利用 SSD1306 自动列递增 */
    OLED_SetCursor(obj, (Line - 1) * 2, (Column - 1) * 8);
    OLED_WriteBulkData(obj, &OLED_F8x16[Char - ' '][0], 8);
    /* 下半部分 (8 像素) */
    OLED_SetCursor(obj, (Line - 1) * 2 + 1, (Column - 1) * 8);
    OLED_WriteBulkData(obj, &OLED_F8x16[Char - ' '][8], 8);
}
